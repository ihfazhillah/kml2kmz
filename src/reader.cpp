...
std::string save_coordinate(std::string coordinate);


int main(){
    coordinates items = get_coordinates(SOURCE_URL);
    ...
    {
        std::string zip_name = label + "/" + label + "_" + std::to_string(i + 1) + ".kmz";
        ...
        // archive_write_set_options(archivePtr, "zip:zip64");
        ...
        // archive_write_disk_set_options(archivePtr, ARCHIVE_EXTRACT_TIME);
//             archive_write_set_options(archivePtr, "zip:compression=store");
// archive_write_set_format_zip(archivePtr);
// archive_write_set_compression_compress(archivePtr);

        // archive_write_add_filter_gzip(archivePtr);

        std::string template_content = save_coordinate(items.items[i]);
        // std::cout << "File content: " << std::endl << template_content <<std::endl ;
...
archive_write_data(archivePtr, template_content.data(), template_content.size()); // Note 3
...
archive_entry_free(entryPtr);
        ...
        // archive_write_header(archivePtr, entryPtr);
        // archive_entry_set_size(entryPtr, sizeof template_content); // Note 3

        // archive_write_data(archivePtr, &template_content, sizeof template_content);
        // //archive_write_finish_entry(archivePtr);
        // // archive_write_finish(archivePtr);
        // archive_entry_free(entryPtr);
...
coordinates get_coordinates(char *filename){
